...
interface AuthorizeAttemptRepository : CoroutineCrudRepository<AuthorizeAttemptEntity, UUID> {
    ...
    suspend fun updateProviderIdProviderNonceJsonWebTokenId(
        @Id id: UUID,
        providerId: String,
        providerNonceJsonWebTokenId: UUID?
    )

    suspend fun updateUserId(@Id id: UUID, userId: UUID)

    suspend fun updateReauthenticationAttemptId(@Id id: UUID, reauthenticationAttemptId: UUID?)
