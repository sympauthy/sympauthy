...
open class InteractiveFlowSessionOAuth2Manager(
    ...
    open suspend fun startOAuth2Session(
        ...
        val entity = InteractiveFlowSessionOAuth2Entity(
            ...
            requestedScopes = (scopes ?: emptyList()).map(EnabledScope::scope).toTypedArray(),
            state = clientState,
            nonce = clientNonce,
            ...
            consentedScopes = consentableScopes?.map(EnabledScope::scope)?.toTypedArray(),
    ...
    suspend fun setConsentedScopes(
        ...
        val consentedScopeIds = consentedScopes.map(EnabledScope::scope)
    ...
    suspend fun setGrantedScopes(
        ...
        val grantedScopeIds = grantedScopes.map(EnabledScope::scope)
