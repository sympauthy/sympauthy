...
sealed class LockKey(
     ...
     *
     * A transaction whose answer turns on which committed account holds one of these values takes it —
     * `ProvisionalAccountManager.promote`, `CollectedClaimManager.applyUpdates`, and the provider link in
     * `InteractiveFlowSessionOAuth2ProviderManager`, which writes no value of its own and takes the key so
     * that what it refuses on is what a promotion committed. None of them holds a row the others could have
     * waited on — a promotion's rows are invisible to a committed reader, and a value nobody holds yet has
     * no row at all — so nothing but a key all of them name serialises them.
     *
     ...
     *
     * A transaction whose answer turns on which committed account one of these identities is linked to
     * takes it — `ProvisionalAccountManager.promote`, the provider link in
