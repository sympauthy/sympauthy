...
class AudienceCollectionManagerTest {
    ...
    fun `listAudiences - Order by identifier before slicing`() = runTest {
        ...
        val result = audienceCollectionManager.listAudiences(CollectionCriteria.NONE, PageParams(page = 0, size = 1))
    ...
    fun `listAudiences - Answer each audience with the number of clients it groups`() = runTest {
        ...
        val result = audienceCollectionManager.listAudiences(CollectionCriteria.NONE, firstPage)
    ...
    @Test
    fun `listAudiences - Keep the audiences a criterion names`() = runTest {
        every { audienceManager.listAudiences() } returns listOf(audience("default"), audience("admin"))
        coEvery { clientManager.countClientsByAudienceId() } returns mapOf("default" to 3)

        val criteria = audienceCollectionManager.capabilities().criteriaOf("client_count.gt" to "0")
        val result = audienceCollectionManager.listAudiences(criteria, firstPage)

        assertEquals(listOf("default"), result.items.map { it.audience.id })
    ...
    fun `findAudienceByIdOrNull - Answer the audience with the number of clients it groups`() = runTest {
        ...
        val result = audienceCollectionManager.findAudienceByIdOrNull("default")
    ...
    fun `findAudienceByIdOrNull - Answer an audience no client names with 0`() = runTest {
        ...
        val result = audienceCollectionManager.findAudienceByIdOrNull("admin")
    ...
    fun `findAudienceByIdOrNull - Return null when no audience matches`() = runTest {
        ...
        assertNull(audienceCollectionManager.findAudienceByIdOrNull("unknown"))
    }
}
