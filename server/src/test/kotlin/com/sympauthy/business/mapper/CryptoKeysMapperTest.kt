...
class CryptoKeysMapperTest {

    private val mapper = Mappers.getMapper(CryptoKeysMapper::class.java)
    ...
    fun `toCryptoKeys - maps all fields`() {
        ...
        assertEquals("crypto-keys-mapper-test", keys.name)
    ...
    fun `toEntity - maps all fields`() {
        ...
        assertEquals("crypto-keys-mapper-test", entity.name)
    ...
    ).apply { name = "crypto-keys-mapper-test" }

    private fun keys(
        publicKey: ByteArray?,
        publicKeyFormat: String? = "X.509"
    ) = CryptoKeys(
        name = "crypto-keys-mapper-test",
        algorithm = "RSA",
        publicKey = publicKey,
        publicKeyFormat = publicKeyFormat,
        privateKey = byteArrayOf(4, 5, 6, 7),
