...
**Data retention, except where a table holds personal data this server chose to record.** The
security context is the one that does, and it ships with the two windows it is kept for and the job
that deletes it; how long a revoked token or a used validation code is kept is still a policy nobody
has set.
